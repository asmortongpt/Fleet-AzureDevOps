...
export function EnhancedFleetMapDashboard() {
  ...
  const stats = [
    { label: 'Active Vehicles', value: '156', icon: Truck, color: colors.teal },
  ...
  return (
    <div className="min-h-screen bg-[var(--surface-0)]">
      {/* Header */}
      <motion.div
        className="border-b border-[var(--border-subtle)] bg-[var(--surface-1)] sticky top-0 z-10"
      ...
      >
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <MapPin className="w-6 h-6" style={{ color: colors.teal }} />
            <div>
              <h1 className="text-2xl font-bold">Live Fleet Tracking</h1>
              <p className="text-sm text-[var(--text-secondary)]">Real-time vehicle monitoring and analytics</p>
            </div>
          </div>
      ...
      >
        {stats.map((stat) => {
          ...
          return (
            <motion.div
              key={stat.label}
              className="rounded-xl p-4 border border-[var(--border-subtle)] bg-[var(--surface-2)] hover:border-[var(--border-default)] transition-colors"
              whileHover={{ y: -4 }}
      ...
      {/* Footer Info */}
      <motion.div
        className="border-t border-[var(--border-subtle)] bg-[var(--surface-1)] py-6"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              <p className="text-xs uppercase tracking-wider font-semibold text-[var(--text-tertiary)] mb-1">Status</p>
            ...
            <div>
              <p className="text-xs uppercase tracking-wider font-semibold text-[var(--text-tertiary)] mb-1">Last Updated</p>
              <p>Just now • Real-time sync enabled</p>
            </div>
            <div>
              <p className="text-xs uppercase tracking-wider font-semibold text-[var(--text-tertiary)] mb-1">Data</p>
              <p>Powered by CTA Fleet Intelligence</p>
            </div>
          </div>
        </div>
      </motion.div>
