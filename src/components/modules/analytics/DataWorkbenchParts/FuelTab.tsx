import { ChartLine, GasPump, TrendUp } from "@phosphor-icons/react"
            ...
            <table className="w-full" role="table" aria-label="Fuel consumption records">
              <thead className="border-b bg-muted/50">
                <tr>
                  ...
                  <th className="text-left p-2 font-medium" scope="col">Station</th>
                  <th className="text-left p-2 font-medium" scope="col">Brand</th>
                  <th className="text-left p-2 font-medium" scope="col">Fill</th>
                  <th className="text-left p-2 font-medium" scope="col">Calc MPG</th>
                </tr>
              </thead>
              <tbody>
                {sortedFuelRecords.slice(0, 20).map(record => {
                  const calcMpg = record.mpgCalculated ?? record.mpg ?? 0
                  const mpgColor = calcMpg >= 25
                  ...
                  <tr key={record.id} className="border-b hover:bg-muted/50 transition-colors">
                    ...
                    <td className="p-2 text-sm text-muted-foreground">
                      {record.stationName || record.location || "-"}
                    </td>
                    ...
                    <td className="p-2">
                      {record.mpgCalculated != null ? (
                        <Badge variant="outline" className={`text-xs ${mpgColor}`}>
                          {record.mpgCalculated.toFixed(1)}
                        </Badge>
                      ) : (
                        <span className="text-muted-foreground text-sm">-</span>
                      )}
