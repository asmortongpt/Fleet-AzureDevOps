...
function ConversationListItem({ conversation, onClick }: ConversationListItemProps) {
  const statusColors = {
    ...
    active: 'text-emerald-400 border-emerald-500',
...
export function AiAgentDrilldown() {
  ...
  const conversations = Array.isArray(rawConversationsData) ? rawConversationsData : []
...
export function MessagesDrilldown() {
  ...
  const messages = Array.isArray(rawMessagesData) ? rawMessagesData : []
  ...
  const channelData = Array.isArray(rawChannelData) ? rawChannelData : []
  const channelActivity = channelData.length > 0 ? channelData : [
          ...
          {channelActivity.map(ch => (
            ...
            >
              <div className="flex items-center gap-3">
                <span className={`w-2 h-2 rounded-full ${ch.active ? 'bg-emerald-400' : 'bg-white/[0.1]'}`} />
                <span className="text-white/80 font-medium">{ch.channel}</span>
                {ch.unread > 0 && (
                  <Badge className="bg-emerald-500 text-white text-xs">{ch.unread}</Badge>
                )}
              </div>
              <div className="flex items-center gap-2">
...
export function EmailDrilldown() {
  ...
  const { data: rawEmails } = useSWR<OutlookMessage[]>(
    `/api/outlook/messages?source=local&top=200`,
    apiFetcher
  )

  const emails: EmailListItemProps['email'][] = (Array.isArray(rawEmails) ? rawEmails : []).map((email) => ({
    id: email.id,
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="grid grid-cols-4 gap-3">
        <Card
          className="bg-white/[0.04] border-emerald-700/50 cursor-pointer hover:border-emerald-500/50 transition-colors"
      ...
      <Card className="bg-[#111111] border-white/[0.04]">
        ...
        <CardContent className="space-y-2">
          {[
            ...
            >
              <div className="flex justify-between text-sm">
                <span className="text-white/80">{campaign.name}</span>
...
export function HistoryDrilldown() {
  ...
  const { data: rawLogs } = useSWR<CommunicationLogRow[]>(
    '/api/communication-logs?limit=200',
    apiFetcher
  )
  const { data: rawHistoryEmails } = useSWR<OutlookMessage[]>(
    '/api/outlook/messages?source=local&top=200',
    apiFetcher
  )
  const logs = Array.isArray(rawLogs) ? rawLogs : []
  const historyEmails = Array.isArray(rawHistoryEmails) ? rawHistoryEmails : []
