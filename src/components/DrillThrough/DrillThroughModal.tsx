...
export function DrillThroughModal({ config, isOpen, onClose }: DrillThroughModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-2">
      <div className="bg-[var(--surface-2)] rounded-lg max-w-7xl w-full max-h-[90vh] flex flex-col">
        {/* Header */}
        ...
        {/* Toolbar */}
        <div className="flex items-center justify-between p-2 border-b border-[var(--border-subtle)] bg-white/[0.02]">
          <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`px-2 py-2 rounded-lg flex items-center gap-2 transition-colors ${
                  showFilters
                    ...
                    : 'bg-white/[0.04] text-[var(--text-secondary)] border border-[var(--border-subtle)]'
                }`}
              >
            ...
            <button
              onClick={() => refetch()}
              disabled={isLoading}
              className="px-2 py-2 rounded-lg flex items-center gap-2 bg-white/[0.04] text-[var(--text-secondary)] border border-[var(--border-subtle)] hover:bg-white/[0.06] transition-colors disabled:opacity-50"
            >
          ...
          {config.enableExport !== false && (
            <div className="flex items-center gap-2">
              <span className="text-sm text-[var(--text-secondary)]">Export:</span>
              <button
        ...
        {/* Summary Stats */}
        {data?.summary && (
          <div className="p-2 bg-emerald-500/5 border-b border-[var(--border-subtle)]">
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-white/[0.04]">
                    {config.columns.map((column) => (
                      <th
                        key={column.key}
                        className={`px-2 py-3 text-left text-sm font-semibold text-white border-b border-[var(--border-subtle)] ${
                ...
                <tbody>
                  {sortedData.map((row, index) => (
                    <tr
                      key={row.id || index}
                      className="hover:bg-[var(--surface-glass-hover)] transition-colors"
                    >
                      {config.columns.map((column) => (
                        <td
                          key={column.key}
                          className="px-2 py-3 text-sm text-[var(--text-primary)] border-b border-[var(--border-subtle)]"
                        >
                          {column.render
        ...
        {data && data.totalPages > 1 && (
          <div className="flex items-center justify-between p-2 border-t border-[var(--border-subtle)] bg-white/[0.02]">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                disabled={page === 1}
                className="px-3 py-2 rounded bg-white/[0.04] text-[var(--text-secondary)] border border-[var(--border-subtle)] hover:bg-white/[0.06] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
