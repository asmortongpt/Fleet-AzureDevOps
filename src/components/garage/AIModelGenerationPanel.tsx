  ...
  return (
    <div className="absolute right-4 top-16 z-30 w-80 bg-[#0e0e0e]/95 backdrop-blur-sm rounded-xl border border-white/[0.04] overflow-hidden">
      ...
      <div className="px-4 pt-3">
        <div className="relative aspect-[16/9] bg-[#0a0a0a] rounded-lg overflow-hidden">
          <img
      ...
      <div className="px-4 py-3 space-y-3">
        ...
        {status === 'queued' && (
          <div className="flex items-center gap-3 py-2">
            ...
            <div>
              <div className="text-xs font-medium text-white/70">Queued for generation...</div>
              <div className="text-[10px] text-white/40 mt-0.5">Estimated: ~2 minutes</div>
            </div>
          </div>
        )}
        ...
        {status === 'processing' && (
          <div className="space-y-2.5">
            <div className="flex items-center justify-between text-[10px]">
              <span className="text-white/60 font-medium">{stage || 'Processing...'}</span>
            ...
            <div className="space-y-1">
              {STAGES.map((s, i) => (
                <div key={s} className="flex items-center gap-2">
                  ...
                  <span className={cn(
                    'text-[9px]',
                    i <= stageIndex ? 'text-white/60' : 'text-white/25'
                  )}>
                    {s}
                  </span>
                </div>
          ...
          <div className="space-y-2.5">
            <div className="flex items-center gap-2 py-1">
              ...
              <div>
                ...
                <div className="text-[10px] text-white/40">Ready to apply</div>
              </div>
            </div>
            <button
              onClick={handleApply}
        ...
        {status === 'error' && (
          <div className="space-y-2.5">
            ...
            <button
              ...
              className="w-full py-2 rounded-lg text-xs font-medium bg-white/[0.08] text-white/60 hover:bg-white/[0.12] transition-colors"
            >
              Retry
            </button>
          </div>
