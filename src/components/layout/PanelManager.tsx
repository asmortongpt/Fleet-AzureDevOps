...
export function PanelManager() {
  ...
  return (
    ...
    >
      {/* Panel header */}
      <div className="flex items-center gap-2 px-4 py-2.5 border-b border-white/[0.06] shrink-0 bg-[#0A0E27]/95">
        {/* Back button */}
        {panelDepth > 1 && (
          <button
            onClick={popPanel}
            className="flex items-center justify-center w-7 h-7 rounded-lg text-white/40 hover:text-white hover:bg-white/[0.06] transition-all"
            aria-label="Go back"
          >
        ...
        <button
          onClick={closeAll}
          className="flex items-center justify-center w-7 h-7 rounded-lg text-white/40 hover:text-white hover:bg-white/[0.06] transition-all shrink-0"
