...
export function OperationsHubMap({
  ...
  return (
    <div className="w-full h-full relative bg-[var(--surface-0)]" data-testid="operations-hub-map">
      ...
      {/* Map Legend */}
      <div className="absolute top-4 left-4 z-20 bg-[var(--surface-2)] rounded-lg p-2 max-w-xs border border-[var(--border-subtle)]">
        <h3 className="font-semibold text-sm mb-3 text-white">Operations Overview</h3>
        <div className="space-y-2 text-xs">
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 bg-green-500 rounded-full"></div>
            <span className="text-[var(--text-secondary)]">Active Dispatch ({enRouteVehicles.length} en route)</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 bg-amber-500 rounded-full"></div>
            <span className="text-[var(--text-secondary)]">Maintenance ({(vehicles as unknown as Vehicle[]).filter((v: Vehicle) => v.status === 'maintenance').length})</span>
          </div>
          {showGeofences && (
            <div className="flex items-center gap-2">
              <Circle className="w-3 h-3 text-amber-500" />
              <span className="text-[var(--text-secondary)]">Geofence Zones</span>
            </div>
          )}
        </div>
      </div>
      ...
      {showRouteOptimization && (
        <div className="absolute bottom-4 left-4 z-20 bg-[var(--surface-2)] rounded-lg p-2 max-w-sm border border-[var(--border-subtle)]">
      ...
      {/* Dispatch Overlay */}
      {showDispatchOverlay && (
        <div className="absolute top-20 left-4 z-10 bg-[var(--surface-2)] rounded-lg p-3 max-w-xs border border-[var(--border-subtle)]">
          <h4 className="font-semibold text-xs text-[var(--text-secondary)] mb-2 flex items-center gap-2">
            <Truck className="h-4 w-4 text-emerald-200" />
            Active Dispatches
          </h4>
          <div className="space-y-1.5">
            {enRouteVehicles.slice(0, 3).map((vehicle: Vehicle) => (
              <div key={vehicle.id} className="flex items-center justify-between text-xs bg-white/[0.03] rounded p-2 border border-[var(--border-subtle)]">
      ...
      {showGeofences && (
        <div className="absolute bottom-4 right-4 z-20 bg-[var(--surface-2)] rounded-lg p-2 max-w-xs border border-[var(--border-subtle)]">
