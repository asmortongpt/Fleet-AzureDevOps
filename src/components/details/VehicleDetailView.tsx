...
export function VehicleDetailView({ vehicle, onClose }: VehicleDetailViewProps) {
  ...
  const ColorSwatch = ({ colorName }: { colorName: string }) => {
    ...
    return (
      <span
        className="inline-block w-4 h-4 rounded-full border border-[var(--border-subtle)] mr-2 align-middle"
  ...
  return (
    <div className="h-full overflow-y-auto">
      {/* Header Section */}
      <div className="bg-[var(--surface-2)] text-white p-4">
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                ...
                <p className="text-xs text-[var(--text-secondary)] font-mono">VIN: {vehicle.vin || '--'}</p>
              </div>
            </div>
            <div className="grid grid-cols-4 gap-4">
              <div>
                <p className="text-[10px] uppercase tracking-wider text-[var(--text-tertiary)] mb-0.5">License Plate</p>
                <p className="text-sm font-semibold text-white">{vehicle.licensePlate || vehicle.license_plate || '--'}</p>
              </div>
              <div>
                <p className="text-[10px] uppercase tracking-wider text-[var(--text-tertiary)] mb-0.5">Odometer</p>
                <p className="text-sm font-semibold text-white">{odometer != null ? formatNumber(Number(odometer)) + ' mi' : '--'}</p>
              </div>
              <div>
                <p className="text-[10px] uppercase tracking-wider text-[var(--text-tertiary)] mb-0.5">Status</p>
                <p className="text-sm font-semibold text-white">{formatEnum(vehicle.status)}</p>
              </div>
              <div>
                <p className="text-[10px] uppercase tracking-wider text-[var(--text-tertiary)] mb-0.5">Ownership</p>
          ...
          {onClose && (
            <Button variant="ghost" onClick={onClose} className="text-[var(--text-secondary)] hover:text-white hover:bg-[var(--surface-glass-hover)]">
                  ...
                  {tags.length > 0 && (
                    <div className="pt-2 mt-1 border-t border-[var(--border-subtle)]">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {tags.map((tag) => (
                          <Badge key={tag} variant="outline" className="text-[10px] px-1.5 py-0 border-[var(--border-subtle)]">
                            {tag}
                          </Badge>
                        ))}
                      </div>
            ...
            <Card className="bg-[var(--surface-2)] border-[var(--border-subtle)]">
              ...
              <CardContent>
                {serviceHistory.length === 0 ? (
                  ...
                  <div className="max-h-[400px] overflow-y-auto border border-[var(--border-subtle)] rounded-lg">
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {maintenanceSchedule.map((item, index) => (
                          <tr key={item.id || index} className="border-b border-[var(--border-subtle)]">
                            <td className="px-3 py-2 font-medium text-foreground">
