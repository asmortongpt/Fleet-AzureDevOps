...
import { useDVIRMutations, type InspectionType, type CreateDVIRInput } from '@/hooks/use-hos-data'
...
export function DVIRInspectionDialog({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const defects = components
      ...
      .map(c => ({
        component: c.name,
        description: c.defectNotes || '',
        severity: c.category === 'critical' ? 'critical' : c.category === 'important' ? 'major' : 'minor'
      }))
    ...
    const dvirData: CreateDVIRInput = {
      ...
      odometer_reading: parseInt(values.odometer_reading, 10),
