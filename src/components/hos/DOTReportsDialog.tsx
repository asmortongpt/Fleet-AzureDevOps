...
export function DOTReportsDialog({ open, onOpenChange, tenantId }: DOTReportsDialogProps) {
  ...
  // Data hooks
  const { data: drivers = [] } = useDrivers({ tenant_id: tenantId })
  const { data: hosLogs = [] } = useHOSLogs({
    tenant_id: tenantId,
    start_date: dateRange.start.toISOString(),
    end_date: dateRange.end.toISOString(),
    driver_id: selectedDriver !== 'all' ? selectedDriver : undefined
  })
  const { data: dvirReports = [] } = useDVIRReports({
    tenant_id: tenantId,
    start_date: dateRange.start.toISOString(),
    end_date: dateRange.end.toISOString(),
    driver_id: selectedDriver !== 'all' ? selectedDriver : undefined
                ...
                <SelectContent>
                  ...
                  {drivers.map((driver) => (
                    <SelectItem key={driver.id} value={driver.id}>
                      {driver.firstName || driver.first_name} {driver.lastName || driver.last_name} ({driver.licenseNumber || driver.license_number})
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
...
function generateHOSSummary(logs: HOSLog[], dateRange: DateRange): HOSSummaryData {
  return {
    ...
    byStatus: {
      ...
      sleeper: logs.filter((l) => l.duty_status === 'sleeper_berth').length,
      driving: logs.filter((l) => l.duty_status === 'driving').length,
      on_duty: logs.filter((l) => l.duty_status === 'on_duty_not_driving').length
    },
...
function generateDVIRSummary(reports: DVIRReport[], dateRange: DateRange): DVIRSummaryData {
  ...
  return {
    ...
    byInspectionType: {
      ...
      en_route: reports.filter((r) => r.inspection_type === 'enroute').length
    },
    reports
  }
}
