...
const MetricCard = React.memo<MetricCardProps>(({
  ...
  return (
    <div className="bg-[#111111] text-card-foreground rounded-lg p-4 border border-white/[0.04]">
...
const ViolationsTab = React.memo<{
  ...
  return (
    <div className="flex flex-col gap-2 h-full overflow-hidden">
      {/* Search and Filters */}
      <div className="bg-[#111111] text-card-foreground rounded-lg p-3 border border-white/[0.04]">
        <div className="flex flex-row gap-2">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-1.5 text-sm border border-white/[0.04] rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent bg-background/70 text-foreground"
              aria-label="Search violations"
            />
          </div>
          <div className="flex gap-2">
            <button
              className="flex items-center gap-1.5 px-3 py-1.5 text-sm border border-white/[0.04] rounded-lg text-muted-foreground"
      ...
      {/* Violations Table */}
      <div className="bg-[#111111] text-card-foreground rounded-lg border border-white/[0.04] overflow-hidden flex-1">
      ...
      <div className="grid grid-cols-3 gap-2 overflow-y-auto max-h-[200px]">
        {!data.data || data.data.reports.length === 0 ? (
          ...
          data.data.reports.map((report) => (
            <div
              key={report.id}
              className="bg-[#111111] text-card-foreground rounded-lg p-4 border border-white/[0.04] cursor-pointer"
              onClick={() => push({
                id: report.id,
                type: 'report',
                label: report.title,
