    ...
    return (
        <div className="p-3 space-y-2 bg-gradient-to-b from-slate-900/50 to-transparent">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
                <StatCard title="Queries Today" value="145" variant="primary" icon={<Sparkle className="w-4 h-4" />} onClick={() => push({ type: 'ai-agent', data: { title: 'AI Queries' } } as Omit<DrilldownLevel, "timestamp">)} />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-2">
                <div className="bg-gradient-to-br from-slate-800/60 to-slate-900/60 backdrop-blur-xl rounded-md border border-slate-700/50 p-3 cursor-pointer hover:border-slate-600/50 transition-colors" onClick={() => push({ type: 'ai-agent', data: { title: 'Resolution Rate' } } as Omit<DrilldownLevel, "timestamp">)}>
                    <h3 className="text-sm font-medium text-slate-400 uppercase tracking-wide mb-2">Resolution Rate</h3>
                    <ProgressRing progress={87} color="green" label="Resolved" sublabel="without escalation" />
                </div>
                <div className="bg-gradient-to-br from-slate-800/60 to-slate-900/60 backdrop-blur-xl rounded-md border border-slate-700/50 p-3 cursor-pointer hover:border-slate-600/50 transition-colors" onClick={() => push({ type: 'ai-agent', data: { title: 'AI Usage' } } as Omit<DrilldownLevel, "timestamp">)}>
                    <h3 className="text-sm font-medium text-slate-400 uppercase tracking-wide mb-2">Usage</h3>
...
function TeamsContent() {
    ...
    return (
        <div className="p-3 space-y-2 bg-gradient-to-b from-slate-900/50 to-transparent">
            <h2 className="text-sm font-bold text-white">Teams Integration</h2>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
                <StatCard title="Connected Users" value="48" variant="primary" icon={<MicrosoftTeamsLogo className="w-4 h-4" />} onClick={() => push({ type: 'messages', data: { title: 'Connected Users' } } as Omit<DrilldownLevel, "timestamp">)} />
...
function EmailContent() {
    ...
    return (
        <div className="p-3 space-y-2 bg-gradient-to-b from-slate-900/50 to-transparent">
            <h2 className="text-sm font-bold text-white">Email Center</h2>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
                <StatCard title="Sent Today" value="156" variant="primary" icon={<PaperPlaneTilt className="w-4 h-4" />} onClick={() => push({ type: 'email', data: { title: 'Emails Sent' } } as Omit<DrilldownLevel, "timestamp">)} />
...
function LogContent() {
    ...
    return (
        <div className="p-3 space-y-2 bg-gradient-to-b from-slate-900/50 to-transparent">
            <h2 className="text-sm font-bold text-white">Communication Log</h2>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
                <StatCard title="Total Messages" value="4,567" variant="primary" icon={<Chat className="w-4 h-4" />} onClick={() => push({ type: 'communication-history', data: { title: 'Total Messages' } } as Omit<DrilldownLevel, "timestamp">)} />
...
export function CommunicationHub() {
    ...
    return (
        <HubPage
            ...
            icon={<CommunicationIcon className="w-4 h-4" />}
