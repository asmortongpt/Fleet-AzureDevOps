    ...
    return (
        <div className="p-4 sm:p-6 space-y-4 sm:space-y-6 bg-gradient-to-b from-background to-background/95 min-h-full overflow-auto">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <div>
                    <h2 className="text-lg sm:text-xl lg:text-2xl font-bold text-foreground">Incident Management</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                <StatCard title="Open Incidents" value="3" variant="danger" icon={<WarningCircle className="w-5 h-5 sm:w-6 sm:h-6" />} onClick={() => push({ type: 'open-incidents', data: { title: 'Open Incidents' } } as Omit<DrilldownLevel, "timestamp">)} />
                ...
                <StatCard title="Days Incident Free" value="47" variant="success" icon={<ShieldCheck className="w-5 h-5 sm:w-6 sm:h-6" />} onClick={() => push({ type: 'days-incident-free', data: { title: 'Safety Record' } } as Omit<DrilldownLevel, "timestamp">)} />
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-4">
                <div className="bg-card/80 backdrop-blur-xl rounded-xl border border-border/50 p-4 sm:p-6 cursor-pointer hover:border-success/40 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300" onClick={() => push({ type: 'safety-score-detail', data: { title: 'Safety Score' } } as Omit<DrilldownLevel, "timestamp">)} role="button" tabIndex={0}>
                    <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-4">Safety Score</h3>
                    <div className="flex items-center justify-center">
                        <ProgressRing progress={92} color="green" label="92%" sublabel="Fleet-wide" />
                    </div>
                </div>
                <div className="bg-card/80 backdrop-blur-xl rounded-xl border border-border/50 p-4 sm:p-6 cursor-pointer hover:border-border transition-all duration-300" onClick={() => push({ type: 'incidents', data: { title: 'Safety Metrics' } } as Omit<DrilldownLevel, "timestamp">)} role="button" tabIndex={0}>
                    <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-4">Safety Metrics</h3>
                ...
                <div className="bg-card/80 backdrop-blur-xl rounded-xl border border-border/50 p-4 sm:p-6 cursor-pointer hover:border-primary/40 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300" onClick={() => push({ type: 'incidents', data: { title: 'Response Time' } } as Omit<DrilldownLevel, "timestamp">)} role="button" tabIndex={0}>
                    <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-4">Response Time</h3>
                    <div className="flex items-center justify-center">
...
function VideoContent() {
    ...
    return (
        <div className="p-4 sm:p-6 space-y-4 sm:space-y-6 bg-gradient-to-b from-background to-background/95 overflow-auto">
            <h2 className="text-lg sm:text-xl lg:text-2xl font-bold text-foreground">Video Telematics</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                <StatCard title="Cameras Online" value="148" variant="success" icon={<VideoCamera className="w-5 h-5 sm:w-6 sm:h-6" />} onClick={() => push({ type: 'cameras-online', data: { title: 'Cameras Online' } } as Omit<DrilldownLevel, "timestamp">)} />
...
function AlertsContent() {
    ...
    return (
        <div className="p-4 sm:p-6 space-y-4 sm:space-y-6 bg-gradient-to-b from-background to-background/95 overflow-auto">
            <h2 className="text-lg sm:text-xl lg:text-2xl font-bold text-foreground">Safety Alerts</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                <StatCard title="Active Alerts" value="6" variant="warning" icon={<Bell className="w-5 h-5 sm:w-6 sm:h-6" />} onClick={() => push({ type: 'safety-alerts', data: { title: 'Active Alerts', filter: 'active' }, id: 'active-alerts' } as Omit<DrilldownLevel, "timestamp">)} />
