        ...
        <div className="p-6 space-y-6 bg-gradient-to-b from-slate-900/50 to-transparent">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <StatCard
                    ...
                    onClick={() => push({ type: 'active-alerts', data: { title: 'Active Alerts' } } as Omit<DrilldownLevel, "timestamp">)}
                />
                <StatCard
                    title="Critical"
                    value="1"
                    variant="danger"
                    onClick={() => push({ type: 'critical-alerts', data: { title: 'Critical Alerts' } } as Omit<DrilldownLevel, "timestamp">)}
                />
                <StatCard
                    title="Acknowledged"
                    value="4"
                    variant="success"
                    onClick={() => push({ type: 'acknowledged-alerts', data: { title: 'Acknowledged Alerts' } } as Omit<DrilldownLevel, "timestamp">)}
                />
                <StatCard
                    title="Auto-Resolved"
                    value="12"
                    variant="default"
                    onClick={() => push({ type: 'auto-resolved-alerts', data: { title: 'Auto-Resolved Alerts' } } as Omit<DrilldownLevel, "timestamp">)}
                />
            </div>
        </div>
    )
