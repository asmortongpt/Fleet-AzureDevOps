...
export const useDemoMode = () => {
  ...
  const enableDemoMode = useCallback((initialRole: UserRole = 'fleet_manager') => {
    ...
    setUser({
      ...
      role: roleMap[initialRole] as 'super-admin' | 'admin' | 'manager' | 'supervisor' | 'technician' | 'driver' | 'viewer',
  ...
  const switchRole = useCallback(async (newRole: UserRole) => {
    ...
    setUser({
      ...
      email: demoUser.email,
      name: `${demoUser.firstName} ${demoUser.lastName}`,
      role: roleMap[newRole] as 'super-admin' | 'admin' | 'manager' | 'supervisor' | 'technician' | 'driver' | 'viewer',
      permissions: ['read', 'write'],
      status: 'active',
      createdAt: new Date().toISOString(),
      token: 'demo-token-' + demoUser.id
