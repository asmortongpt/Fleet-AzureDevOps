...
export function useReactiveMaintenanceData() {
  ...
  if (USE_MOCK_DATA) {
    // Transform mock work orders to match expected format
    const workOrders = mockData.workOrders.map((wo, idx) => ({
      id: `wo-${idx + 1}`,
      vehicleId: `vehicle-${idx + 1}`,
      vehicleName: wo.vehicle,
      type: wo.type,
      status: wo.status as 'scheduled' | 'in_progress' | 'completed' | 'cancelled' | 'parts_waiting',
      priority: wo.priority as 'low' | 'medium' | 'high' | 'urgent',
      estimatedHours: wo.estimated_cost ? wo.estimated_cost / 50 : 2, // Estimate hours from cost
      actualHours: wo.actual_cost ? wo.actual_cost / 50 : undefined,
      description: wo.notes || '',
      assignedTo: `tech-${idx + 1}`,
      assignedToName: wo.technician,
      createdAt: new Date(wo.scheduled_date).toISOString(),
      updatedAt: new Date().toISOString(),
      completedAt: wo.status === 'completed' ? new Date(wo.scheduled_date).toISOString() : undefined,
      cost: wo.actual_cost || wo.estimated_cost,
    }))

    const metrics = {
      totalWorkOrders: mockData.maintenanceStats.total_work_orders,
      urgentOrders: workOrders.filter(wo => wo.priority === 'urgent').length,
      inProgress: mockData.maintenanceStats.in_progress,
      completed: mockData.maintenanceStats.completed,
      partsWaiting: 0,
      avgCompletionTime: 2.5,
    }

    const statusDistribution: Record<string, number> = {
      scheduled: mockData.maintenanceStats.scheduled,
      in_progress: mockData.maintenanceStats.in_progress,
      completed: mockData.maintenanceStats.completed,
      cancelled: 0,
      parts_waiting: 0,
    }
    ...
    workOrders.forEach(wo => {
      typeDistribution[wo.type] = (typeDistribution[wo.type] || 0) + 1
    })
    ...
    const requestTrendData = Array.from({ length: 7 }, (_, i) => ({
      name: new Date(Date.now() - (6 - i) * 24 * 60 * 60 * 1000).toLocaleDateString('en-US', { weekday: 'short' }),
      count: Math.floor(Math.random() * 5) + 1,
    }))

    const urgentWorkOrders = workOrders.filter(wo => wo.priority === 'urgent')

    const highConfidencePredictions = [
      {
        id: 'pred-1',
        vehicleId: 'vehicle-3',
        vehicleName: 'Ram 1500 (FL-2024-203)',
        issue: 'Transmission wear detected',
      ...
      },
      {
        id: 'pred-2',
        vehicleId: 'vehicle-2',
        vehicleName: 'Chevrolet Silverado (FL-2023-102)',
        issue: 'Brake pad degradation',
