...
import { checkUserPermission, validateGPS, stripEXIFData, rateLimiter } from '../utils/securityUtils';
...
router.post('/checkout', verifyJWT, rateLimiter(10), upload.single('photo'), async (req: Request, res: Response) => {
  try {
    const { assetId, userId, tenantId, gpsLat, gpsLng, conditionRating, digitalSignature } = assetActionSchema.parse(req.body);
    ...
    const queryText = `
      ...
      RETURNING *;
    `;
    const values = [assetId, userId, tenantId, gpsLat, gpsLng, conditionRating, digitalSignature, cleanPhoto];
...
router.post('/checkin', verifyJWT, rateLimiter(10), upload.single('photo'), async (req: Request, res: Response) => {
  try {
    const { assetId, userId, tenantId, gpsLat, gpsLng, conditionRating, digitalSignature } = assetActionSchema.parse(req.body);
    const photo = req.file;
