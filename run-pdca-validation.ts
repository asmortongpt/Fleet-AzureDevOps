...
async function runPDCAValidation() {
  console.log('\n' + '='.repeat(80));
  console.log('🔍 PDCA LOOP: Fleet Hub Functionality Validation');
    ...
    for (const hubExpectation of EXPECTED_HUBS) {
      ...
      try {
        ...
        if (!sidebarExists) {
          ...
          console.log('  ❌ Right sidebar: Missing');
        } else {
...
runPDCAValidation().catch(console.error);
