...
-- Complete Data Update Script (Fixed)
-- ============================================================================
...
-- Update drivers with Tallahassee phone numbers (already done, verify)
...
UPDATE drivers d
SET
  phone = '850-' || LPAD(((du.rn * 17 + 100)::text), 3, '0') || '-' || LPAD(((du.rn * 23 + 1000)::text), 4, '0'),
  updated_at = NOW()
FROM driver_updates du
WHERE d.id = du.id
  AND (d.phone IS NULL OR d.phone NOT LIKE '850-%');

COMMIT;

-- Verification queries
\echo '========================================='
\echo 'Tallahassee Fleet Data Summary'
\echo '========================================='

SELECT
  COUNT(*) as total_vehicles,
  COUNT(CASE WHEN latitude IS NOT NULL AND longitude IS NOT NULL THEN 1 END) as with_gps,
  COUNT(DISTINCT assigned_driver_id) as with_drivers,
  COUNT(CASE WHEN last_service_date IS NOT NULL THEN 1 END) as with_service_dates,
  COUNT(CASE WHEN number LIKE 'TLH-%' THEN 1 END) as tallahassee_numbered
FROM vehicles;

\echo ''
\echo 'Sample Vehicles:'
SELECT number, name, location_address, assigned_driver_id IS NOT NULL as has_driver
FROM vehicles
...
\echo ''
\echo 'Tallahassee Routes:'
SELECT COUNT(*) as total_routes,
       COUNT(CASE WHEN name LIKE '%FSU%' OR name LIKE '%FAMU%' OR name LIKE '%Tallahassee%' OR name LIKE '%Downtown%' THEN 1 END) as tallahassee_routes
FROM routes;
...
FROM routes
WHERE name LIKE '%FSU%' OR name LIKE '%FAMU%' OR name LIKE '%Downtown%'
LIMIT 5;

\echo ''
\echo 'Drivers with 850 Phone:'
SELECT COUNT(*) as drivers_with_850_phone
FROM drivers
