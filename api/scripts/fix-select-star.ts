...
/**
 * Automated SELECT * Query Elimination Script
 *
 * This script:
 * 1. Extracts table schemas from migration files
 * 2. Finds all SELECT * queries in TypeScript files
 * 3. Replaces them with explicit column lists
...
// Total tables: 338
  ...
  // Check for template literal ${this.tableName}
  if (query.includes('${this.tableName}')) {
    return 'dynamic'; // Will need manual handling
  }
...
/**
 * Find all SELECT * queries in the codebase
 */
function findAllSelectStarQueries(): QueryOccurrence[] {
  ...
  for (const line of lines) {
    ...
    const query = queryMatch[0].slice(1, -1); // Remove quotes
...
/**
 * Generate explicit column list for a table
 */
function getExplicitColumns(tableName: string): string {
  ...
  // Format columns nicely with proper indentation
  if (columns.length <= 3) {
    return columns.join(', ');
  }
...
/**
 * Replace SELECT * with explicit columns in a query
 */
