...
// Total tables: 338
  ...
  quality_gates: ['id', 'tenant_id', 'gate_name', 'gate_type', 'criteria', 'threshold', 'is_required', 'created_at', 'updated_at'],
  vehicle_assignments: ['id', 'tenant_id', 'vehicle_id', 'driver_id', 'assignment_type', 'start_date', 'end_date', 'status', 'created_at', 'updated_at'],
  on_call_periods: ['id', 'tenant_id', 'user_id', 'start_time', 'end_time', 'is_active', 'created_at', 'updated_at'],
  trip_gps_breadcrumbs: ['id', 'trip_id', 'latitude', 'longitude', 'accuracy', 'speed', 'heading', 'altitude', 'recorded_at'],
  trip_obd2_metrics: ['id', 'trip_id', 'metric_type', 'value', 'unit', 'recorded_at'],
  safety_incidents: ['id', 'tenant_id', 'incident_type', 'severity', 'description', 'vehicle_id', 'driver_id', 'incident_date', 'status', 'created_at', 'updated_at'],
...
function fixFileQueries(filePath: string, occurrences: QueryOccurrence[]): number {
  ...
  for (const occurrence of fileOccurrences) {
    ...
    if (oldQuery !== newQuery) {
      ...
      fixCount++;
      console.log(`✅ ${occurrence.tableName} in ${filePath.split('/').pop()}:${occurrence.lineNumber}`);
    }
  }
...
function main() {
  ...
  console.log('\n' + '='.repeat(60));
  console.log(`✅ Fixed ${totalFixed} queries in ${filesModified} files`);
