...
-- FedRAMP Compliant with Audit Logging and Multi-tenancy
...
CREATE TABLE audit_logs (
    ...
    resource_type VARCHAR(100) NOT NULL, -- vehicles, work_orders, etc.
    resource_id UUID,
    ...
    error_message TEXT,
    hash VARCHAR(64), -- SHA-256 hash for integrity (FedRAMP AU-9)
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE facilities (
    ...
    tenant_id UUID REFERENCES tenants(id) ON DELETE CASCADE,
    name VARCHAR(255) NOT NULL,
    facility_type VARCHAR(50), -- garage, depot, service_center
    address VARCHAR(255),
    city VARCHAR(100),
    state VARCHAR(2),
    zip_code VARCHAR(10),
...
CREATE TABLE routes (
    ...
    waypoints JSONB DEFAULT '[]', -- Array of {lat, lng, address, order}
...
CREATE TABLE geofences (
    ...
    tenant_id UUID REFERENCES tenants(id) ON DELETE CASCADE,
    name VARCHAR(255) NOT NULL,
    geofence_type VARCHAR(50), -- circular, polygon
    geometry GEOGRAPHY(POLYGON, 4326), -- PostGIS polygon
    center_latitude DECIMAL(10,8),
    center_longitude DECIMAL(11,8),
    radius DECIMAL(10,2), -- meters (for circular geofences)
...
CREATE TABLE telemetry_data (
    ...
    dtc_codes VARCHAR(10)[], -- Diagnostic Trouble Codes
...
-- ============================================
-- Inspections (Pre-trip, Post-trip, Safety)
-- ============================================
...
CREATE TABLE inspection_forms (
    ...
    form_type VARCHAR(50), -- pre_trip, post_trip, safety, custom
    form_template JSONB NOT NULL, -- {fields: [{name, type, required, options}]}
    is_active BOOLEAN DEFAULT true,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE inspections (
    ...
    form_data JSONB NOT NULL, -- Responses to form fields
    defects_found TEXT,
...
-- Damage Reports with 3D Model Integration
...
CREATE TABLE safety_incidents (
    ...
    incident_type VARCHAR(100), -- accident, injury, near_miss, property_damage
...
CREATE TABLE video_events (
    ...
    event_type VARCHAR(50), -- harsh_braking, harsh_acceleration, collision, speeding, distraction
...
CREATE TABLE charging_stations (
    ...
    station_type VARCHAR(50), -- level_1, level_2, dc_fast_charge
    location VARCHAR(255),
    latitude DECIMAL(10,8),
    longitude DECIMAL(11,8),
...
CREATE TABLE charging_sessions (
    ...
    cost DECIMAL(10,2),
    start_battery_level DECIMAL(5,2), -- percentage
    end_battery_level DECIMAL(5,2), -- percentage
...
CREATE TABLE vendors (
    ...
    vendor_type VARCHAR(50), -- parts_supplier, fuel_provider, service_provider
...
CREATE TABLE purchase_orders (
    ...
    line_items JSONB DEFAULT '[]', -- [{part_number, description, quantity, unit_price, total}]
...
CREATE TABLE communication_logs (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    tenant_id UUID REFERENCES tenants(id) ON DELETE CASCADE,
    communication_type VARCHAR(50), -- email, sms, phone, in_person
...
CREATE TABLE policies (
    ...
    policy_type VARCHAR(50), -- speed_limit, idle_time, maintenance, fuel_efficiency
    description TEXT,
    rules JSONB NOT NULL, -- {conditions: [], actions: []}
    is_active BOOLEAN DEFAULT true,
...
CREATE TABLE notifications (
    ...
    tenant_id UUID REFERENCES tenants(id) ON DELETE CASCADE,
    user_id UUID REFERENCES users(id) ON DELETE CASCADE,
    notification_type VARCHAR(50), -- alert, reminder, info
...
DECLARE
    ...
    v_hash VARCHAR(64);
BEGIN
    -- Generate hash for integrity (simple example - in production use proper chaining)
...
-- Active vehicles with current location
...
INSERT INTO schema_version (version, description) VALUES (1, 'Initial schema with FedRAMP compliance and full fleet management features');
