...
export class PolicyRepository extends BaseRepository<Policy> {
  ...
  async findByTenantId(tenantId: number): Promise<Policy[]> {
    const query = `
  ...
  async findByIdAndTenant(id: number, tenantId: number): Promise<Policy | null> {
    const query = `
      SELECT id, name, created_at, updated_at, tenant_id FROM ${this.tableName}
  ...
  async create(data: Partial<Policy>): Promise<Policy> {
    const { columnNames, placeholders, values } = buildInsertClause(data, [], 1, 'policies')
  ...
  async update(id: number, tenantId: number, data: Partial<Policy>): Promise<Policy | null> {
    const { fields: setClause, values } = buildUpdateClause(data, 3, 'policies')
