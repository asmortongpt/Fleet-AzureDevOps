...
export class MobileTelemetryRepository {
  ...
  async findAll(tenantId: number): Promise<MobileTelemetry[]> {
    const query = 'SELECT * FROM mobile_telemetry WHERE tenant_id = $1 AND deleted_at IS NULL ORDER BY timestamp DESC';
    try {
  ...
  async findById(tenantId: number, id: number): Promise<MobileTelemetry | null> {
    const query = 'SELECT * FROM mobile_telemetry WHERE tenant_id = $1 AND id = $2 AND deleted_at IS NULL';
    try {
