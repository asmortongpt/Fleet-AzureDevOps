...
export class TelemetryRepository extends BaseRepository<Telemetry> {
  ...
  async findByTenantId(tenantId: number): Promise<Telemetry[]> {
    const query = `
  ...
  async findByIdAndTenant(id: number, tenantId: number): Promise<Telemetry | null> {
    const query = `
      SELECT * FROM ${this.tableName}
  ...
  async update(id: number, tenantId: number, data: Partial<Telemetry>): Promise<Telemetry | null> {
