...
export class CostRepository extends BaseRepository<Cost> {
  ...
  async findByTenantId(tenantId: number): Promise<Cost[]> {
    const query = `
  ...
  async findByIdAndTenant(id: number, tenantId: number): Promise<Cost | null> {
    const query = `
      SELECT id, name, created_at, updated_at, tenant_id FROM ${this.tableName}
  ...
  async create(data: Partial<Cost>): Promise<Cost> {
    const { columnNames, placeholders, values } = buildInsertClause(data, [], 1, 'costs')
  ...
  async update(id: number, tenantId: number, data: Partial<Cost>): Promise<Cost | null> {
    const { fields: setClause, values } = buildUpdateClause(data, 3, 'costs')
