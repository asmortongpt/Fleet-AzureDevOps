...
async findWithRelatedData(id: string, tenantId: string) {
  const query = \`
    SELECT t.*
    FROM aichat t
    WHERE t.id = \api/src/repositories/aichat.repository.ts AND t.tenant_id = \ AND t.deleted_at IS NULL
  \`;
...
async findAllWithRelatedData(tenantId: string) {
  const query = \`
    SELECT t.*
    FROM aichat t
    WHERE t.tenant_id = \api/src/repositories/aichat.repository.ts AND t.deleted_at IS NULL
