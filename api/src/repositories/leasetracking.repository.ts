...
export class LeaseTrackingRepository {
  ...
  async readAll(tenant_id: string): Promise<LeaseTracking[]> {
    const result = await this.pool.query(
      `SELECT id, tenant_id, vehicle_id, lease_provider, start_date, end_date, monthly_payment, status, created_at, updated_at FROM lease_tracking WHERE tenant_id = $1 ORDER BY created_at DESC`,
      [tenant_id]
    );
    return result.rows;
  }
  ...
  async readById(id: string, tenant_id: string): Promise<LeaseTracking | null> {
    const result = await this.pool.query(
      `SELECT id, tenant_id, vehicle_id, lease_provider, start_date, end_date, monthly_payment, status, created_at, updated_at FROM lease_tracking WHERE id = $1 AND tenant_id = $2`,
      [id, tenant_id]
    );
  ...
  async query1(param1: string, tenant_id: string): Promise<LeaseTracking[]> {
    const result = await this.pool.query(
      `SELECT id, tenant_id, vehicle_id, lease_provider, start_date, end_date, monthly_payment, status, created_at, updated_at FROM lease_tracking WHERE tenant_id = $1 AND status = $2`,
      [tenant_id, param1]
    );
    return result.rows;
  }
  ...
  async query2(param1: string, param2: string, tenant_id: string): Promise<LeaseTracking[]> {
    const result = await this.pool.query(
      `SELECT id, tenant_id, vehicle_id, lease_provider, start_date, end_date, monthly_payment, status, created_at, updated_at FROM lease_tracking
