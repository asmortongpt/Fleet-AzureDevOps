...
export class DeploymentRepository extends BaseRepository<Deployment> {
  constructor(pool: Pool) {
    super(pool, 'deployments')
  }

  async findByTenantId(tenantId: number): Promise<Deployment[]> {
    const query = `
  ...
  async findByIdAndTenant(id: number, tenantId: number): Promise<Deployment | null> {
    const query = `
      SELECT id, created_at, updated_at FROM ${this.tableName}
  ...
  async create(data: Partial<Deployment>): Promise<Deployment> {
    const { columnNames, placeholders, values } = buildInsertClause(data, [], 1, 'deployments')
  ...
  async update(id: number, tenantId: number, data: Partial<Deployment>): Promise<Deployment | null> {
    const { fields: setClause, values } = buildUpdateClause(data, 3, 'deployments')
