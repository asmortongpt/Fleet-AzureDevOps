...
export class FacilityRepository extends BaseRepository<Facility> {
  ...
  async findByTenantId(tenantId: number): Promise<Facility[]> {
    const query = `
  ...
  async findByIdAndTenant(id: number, tenantId: number): Promise<Facility | null> {
    const query = `
      SELECT id, name, created_at, updated_at, tenant_id FROM ${this.tableName}
  ...
  async create(data: Partial<Facility>): Promise<Facility> {
    const { columnNames, placeholders, values } = buildInsertClause(data, [], 1, 'facilities')
  ...
  async update(id: number, tenantId: number, data: Partial<Facility>): Promise<Facility | null> {
    const { fields: setClause, values } = buildUpdateClause(data, 3, 'facilities')
