...
export class DocumentRepository extends BaseRepository<Document> {
  ...
  async create(data: Partial<Document>): Promise<Document> {
    const { columnNames, placeholders, values } = buildInsertClause(data, [], 1, 'documents')
  ...
  async update(id: number, tenantId: number, data: Partial<Document>): Promise<Document | null> {
    const { fields: setClause, values } = buildUpdateClause(data, 3, 'documents')
