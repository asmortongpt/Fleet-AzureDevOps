...
async findWithRelatedData(id: string, tenantId: string) {
  const query = \`
    SELECT t.*
    FROM notes t
    WHERE t.id = \api/src/repositories/notes.repository.ts AND t.tenant_id = \ AND t.deleted_at IS NULL
  \`;
...
async findAllWithRelatedData(tenantId: string) {
  const query = \`
    SELECT t.*
    FROM notes t
    WHERE t.tenant_id = \api/src/repositories/notes.repository.ts AND t.deleted_at IS NULL
