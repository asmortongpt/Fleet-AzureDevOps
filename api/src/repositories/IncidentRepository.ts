...
export class IncidentRepository extends BaseRepository<Incident> {
  ...
  async findByTenantId(tenantId: number): Promise<Incident[]> {
    const query = `
  ...
  async findByIdAndTenant(id: number, tenantId: number): Promise<Incident | null> {
    const query = `
      SELECT id, name, created_at, updated_at, tenant_id FROM ${this.tableName}
  ...
  async create(data: Partial<Incident>): Promise<Incident> {
    const { columnNames, placeholders, values } = buildInsertClause(data, [], 1, 'incidents')
  ...
  async update(id: number, tenantId: number, data: Partial<Incident>): Promise<Incident | null> {
    const { fields: setClause, values } = buildUpdateClause(data, 3, 'incidents')
