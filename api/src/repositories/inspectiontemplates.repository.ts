...
async findWithRelatedData(id: string, tenantId: string) {
  const query = \`
    SELECT t.*
    FROM inspectiontemplates t
    WHERE t.id = \api/src/repositories/inspectiontemplates.repository.ts AND t.tenant_id = \ AND t.deleted_at IS NULL
  \`;
  const result = await this.pool.query(query, [id, tenantId]);
  return result.rows[0] || null;
}
...
async findAllWithRelatedData(tenantId: string) {
  const query = \`
    SELECT t.*
    FROM inspectiontemplates t
    WHERE t.tenant_id = \api/src/repositories/inspectiontemplates.repository.ts AND t.deleted_at IS NULL
