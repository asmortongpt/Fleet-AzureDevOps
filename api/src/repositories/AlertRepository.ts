...
export class AlertRepository extends BaseRepository<Alert> {
  ...
  async findByTenantId(tenantId: number): Promise<Alert[]> {
    const query = `
  ...
  async findByIdAndTenant(id: number, tenantId: number): Promise<Alert | null> {
    const query = `
      SELECT id, created_at, updated_at FROM ${this.tableName}
  ...
  async create(data: Partial<Alert>): Promise<Alert> {
    const { columnNames, placeholders, values } = buildInsertClause(data, [], 1, 'alerts')
  ...
  async update(id: number, tenantId: number, data: Partial<Alert>): Promise<Alert | null> {
    const { fields: setClause, values } = buildUpdateClause(data, 3, 'alerts')
