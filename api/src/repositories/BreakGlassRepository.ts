...
export class BreakGlassRepository extends BaseRepository<BreakGlassSession> {
   ...
   * SECURITY: Validates role belongs to tenant
   */
   ...
   * SECURITY: Filters by tenant_id through user join
   */
   ...
   */
  async findSessionByIdWithTenant(sessionId: string, context: QueryContext): Promise<(BreakGlassSession & { tenant_id: string }) | null> {
    const result = await this.pool.query(
   ...
   */
  async createTemporaryUserRole(userId: string, roleId: string, assignedBy: string, expiresAt: Date): Promise<void> {
    await this.pool.query(
   ...
   */
  async findActiveElevations(userId: string, context: QueryContext): Promise<BreakGlassSessionWithDetails[]> {
    const result = await this.pool.query(
      `SELECT
         bg.*,
         r.name as role_name,
