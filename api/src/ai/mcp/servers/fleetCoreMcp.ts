...
export const fleetCoreMcpServer: MCPServer = {
  ...
  async call(toolName, args, userCtx) {
    if (toolName === "listVehicles") {
return { ok: true, result: { vehicles: [] } };
}
    if (toolName === "getVehicle") {
return { ok: true, result: { vehicle: null } };
}
    if (toolName === "getWorkOrders") {
return { ok: true, result: { workOrders: [] } };
}
    if (toolName === "createWorkOrder") {
return { ok: true, result: { id: "TODO" } };
}
