...
export class RecurringMaintenanceService {
  ...
  async calculateNextDueDate(
    ...
    switch (pattern.type) {
      case 'time':
        return this.addTimeInterval(nextDue, pattern)

      case 'mileage':
        // Estimate based on average daily mileage
        if (vehicleData?.odometer_reading && schedule.current_mileage) {
          const avgMilesPerDay = await this.getAverageDailyMileage(schedule.vehicle_id, schedule.tenant_id)
      ...
      case 'engine_hours':
        ...
        const avgHoursPerDay = await this.getAverageEngineHoursPerDay(schedule.vehicle_id, schedule.tenant_id)
      ...
      case 'combined':
        ...
        const timeBasedDue = this.addTimeInterval(new Date(currentDate), pattern)
        const mileageBasedDue = await this.calculateNextDueDate(
   ...
   */
  private addTimeInterval(date: Date, pattern: RecurrencePattern): Date {
   ...
   */
  private async getAverageDailyMileage(vehicleId: string, tenantId: string): Promise<number> {
   ...
   */
  private async getAverageEngineHoursPerDay(vehicleId: string, tenantId: string): Promise<number> {
    const result = await this.db.query(
    `
    SELECT
