...
CREATE INDEX IF NOT EXISTS idx_routes_assigned_vehicle_id ON routes(assigned_vehicle_id);
-- CREATE INDEX IF NOT EXISTS idx_damage_reports_vehicle_id ON damage_reports(vehicle_id);
-- CREATE INDEX IF NOT EXISTS idx_video_events_vehicle_id ON video_events(vehicle_id);

-- Driver relationships
CREATE INDEX IF NOT EXISTS idx_routes_assigned_driver_id ON routes(assigned_driver_id);
CREATE INDEX IF NOT EXISTS idx_fuel_transactions_driver_id ON fuel_transactions(driver_id);
-- CREATE INDEX IF NOT EXISTS idx_safety_incidents_driver_id ON safety_incidents(driver_id);
...
CREATE INDEX IF NOT EXISTS idx_work_orders_scheduled_start_date ON work_orders(scheduled_start_date);
CREATE INDEX IF NOT EXISTS idx_work_orders_actual_end_date ON work_orders(actual_end_date);
...
CREATE INDEX IF NOT EXISTS idx_routes_scheduled_start_time ON routes(scheduled_start_time DESC);
...
CREATE INDEX IF NOT EXISTS idx_vehicles_status ON vehicles(status) WHERE status != 'retired';
...
CREATE INDEX IF NOT EXISTS idx_work_orders_priority ON work_orders(priority) WHERE priority IN ('high', 'critical');
...
CREATE INDEX IF NOT EXISTS idx_vehicles_make_lower ON vehicles(LOWER(make));
CREATE INDEX IF NOT EXISTS idx_vehicles_model_lower ON vehicles(LOWER(model));
-- CREATE INDEX IF NOT EXISTS idx_drivers_name_lower ON drivers(LOWER(first_name || ' ' || last_name));

-- GIN indexes for JSONB columns
CREATE INDEX IF NOT EXISTS idx_vehicles_metadata_gin ON vehicles USING GIN (metadata);
CREATE INDEX IF NOT EXISTS idx_work_orders_metadata_gin ON work_orders USING GIN (metadata);
...
-- CREATE INDEX IF NOT EXISTS idx_vehicles_location_gist ON vehicles USING GIST (last_location);
-- CREATE INDEX IF NOT EXISTS idx_routes_start_location_gist ON routes USING GIST (start_location);
-- CREATE INDEX IF NOT EXISTS idx_routes_end_location_gist ON routes USING GIST (end_location);
-- CREATE INDEX IF NOT EXISTS idx_geofences_geometry_gist ON geofences USING GIST (geometry);
...
-- Vehicle queries
CREATE INDEX IF NOT EXISTS idx_vehicles_tenant_status_created
    ON vehicles(tenant_id, status, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_vehicles_tenant_type_status
    ON vehicles(tenant_id, type, status);

-- Work order queries
CREATE INDEX IF NOT EXISTS idx_work_orders_tenant_vehicle_status
    ON work_orders(tenant_id, vehicle_id, status);

CREATE INDEX IF NOT EXISTS idx_work_orders_tenant_priority_scheduled
    ON work_orders(tenant_id, priority, scheduled_start_date);

CREATE INDEX IF NOT EXISTS idx_work_orders_tenant_status_scheduled
    ON work_orders(tenant_id, status, scheduled_start_date)
    WHERE status != 'completed';

-- Fuel transaction queries
CREATE INDEX IF NOT EXISTS idx_fuel_tenant_vehicle_date
    ON fuel_transactions(tenant_id, vehicle_id, transaction_date DESC);

CREATE INDEX IF NOT EXISTS idx_fuel_tenant_date_type
    ON fuel_transactions(tenant_id, transaction_date DESC, fuel_type);

-- Route queries
CREATE INDEX IF NOT EXISTS idx_routes_tenant_driver_start
    ON routes(tenant_id, assigned_driver_id, scheduled_start_time DESC);

CREATE INDEX IF NOT EXISTS idx_routes_tenant_vehicle_start
    ON routes(tenant_id, assigned_vehicle_id, scheduled_start_time DESC);

-- Inspection queries
CREATE INDEX IF NOT EXISTS idx_vehicle_inspections_tenant_vehicle_date
    ON vehicle_inspections(tenant_id, vehicle_id, inspected_at DESC);
...
-- Overdue work orders
-- CREATE INDEX IF NOT EXISTS idx_work_orders_overdue
--     ON work_orders(tenant_id, scheduled_start_date)
--     WHERE status != 'completed' AND scheduled_start_date < CURRENT_DATE;

-- High priority items
CREATE INDEX IF NOT EXISTS idx_work_orders_high_priority
    ON work_orders(tenant_id, scheduled_start_date, created_at DESC)
...
-- Maintenance costs
CREATE INDEX IF NOT EXISTS idx_work_orders_cost
    ON work_orders(tenant_id, vehicle_id, actual_cost)
    WHERE actual_cost IS NOT NULL;

-- Fuel efficiency
CREATE INDEX IF NOT EXISTS idx_fuel_gallons_cost
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_vehicles_vin_tenant
    ON vehicles(tenant_id, vin) WHERE vin IS NOT NULL;

-- Prevent duplicate license plates within tenant
CREATE UNIQUE INDEX IF NOT EXISTS idx_vehicles_license_tenant
...
-- CREATE INDEX IF NOT EXISTS idx_vehicles_age
--     ON vehicles(tenant_id, (EXTRACT(YEAR FROM CURRENT_DATE) - year));

-- Days since last inspection
-- CREATE INDEX IF NOT EXISTS idx_vehicles_last_inspection_days
...
CREATE INDEX IF NOT EXISTS idx_vehicles_list_covering
    ON vehicles(tenant_id, status, created_at DESC)
    INCLUDE (id, vin, make, model, year, license_plate);

-- Work order list queries (covering index)
CREATE INDEX IF NOT EXISTS idx_work_orders_list_covering
    ON work_orders(tenant_id, status, priority, scheduled_start_date)
