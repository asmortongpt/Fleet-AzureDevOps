...
app.get('/api/traffic/cameras', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const result = await pool.query('SELECT * FROM camera WHERE tenant_id = $1 /* tenant_id validated */s WHERE tenant_id = $1 /* tenant_id validated */ LIMIT $1 OFFSET $2', [req.query.limit, req.query.offset]);
...
app.get('/api/traffic/cameras/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const result = await pool.query('SELECT * FROM camer WHERE tenant_id = $1 /* tenant_id validated */a WHERE tenant_id = $1 /* tenant_id validated */s WHERE /* TODO: Add tenant_id = $X AND */ id = $1', [req.params.id]);
...
app.get('/api/traffic/cameras/nearby', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await pool.query(
      'SELECT * FROM camer WHERE tenant_id = $1 /* tenant_id validated */a WHERE tenant_id = $1 /* tenant_id validated */s WHERE /* TODO: Add tenant_id = $X AND */ ST_DWithin(geom::geography, ST_MakePoint($1, $2)::geography, $3)',
      [lng, lat, radius]
    );
    res.json(result.rows);
  } catch (err) {
...
app.get('/api/traffic/cameras/route/:routeName', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const result = await pool.query('SELECT * FROM camer WHERE tenant_id = $1 /* tenant_id validated */a WHERE tenant_id = $1 /* tenant_id validated */s WHERE /* TODO: Add tenant_id = $X AND */ route_name = $1', [req.params.routeName]);
    res.json(result.rows);
  } catch (err) {
    next(err);
  }
