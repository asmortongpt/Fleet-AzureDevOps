...
const preferencesSchema = z.object({
  ...
  quietHoursStart: z.string().optional().regex(/^([01]\d|2[0-3]):([0-5]\d)$/),
  quietHoursEnd: z.string().optional().regex(/^([01]\d|2[0-3]):([0-5]\d)$/)
});
 ...
 */
router.put('/preferences', validateSchema(preferencesSchema), asyncHandler(async (req: Request, res: Response) => {
  const userId = req.user!.id;
  const {
    emailEnabled,
    smsEnabled,
