...
async findWithRelatedData(id: string, tenantId: string) {
  const query = \`
    SELECT t.*
    FROM incident_witness t
    WHERE t.id = \api/src/modules/incidents/repositories/incident-witness.repository.ts AND t.tenant_id = \ AND t.deleted_at IS NULL
  \`;
...
async findAllWithRelatedData(tenantId: string) {
  const query = \`
    SELECT t.*
    FROM incident_witness t
    WHERE t.tenant_id = \api/src/modules/incidents/repositories/incident-witness.repository.ts AND t.deleted_at IS NULL
