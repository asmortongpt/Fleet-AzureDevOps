...
const pool = new Pool({
  ...
  // DEV-ONLY script: rejectUnauthorized: false is acceptable for local seed data generation
...
async function seedUltraFast() {
  ...
  try {
    ...
    const driverStatuses: string[] = ['active', 'on_leave', 'suspended']; // Explicit type
    const cdlClasses: (string | null)[] = ['A', 'B', 'C', null]; // Explicit type
    ...
    for (const tenant of tenants) {
      ...
      for (let i = 0; i < facilityCount; i++) {
        ...
        const type = randomItem(['garage', 'depot', 'service_center'] as string[]);
        facilityValues.push(`(
    ...
    const vehicleStatuses: string[] = ['active', 'maintenance', 'out_of_service'];
    ...
    const workOrderStatuses: string[] = ['open', 'in_progress', 'completed'];
