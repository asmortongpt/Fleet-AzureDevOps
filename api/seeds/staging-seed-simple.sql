-- ============================================
-- STAGING ENVIRONMENT SEED DATA (Schema Compatible)
...
SELECT u.tenant_id, u.id, 'FL' || LPAD((100000 + u.id)::TEXT, 8, '0'), 'FL', CURRENT_DATE + INTERVAL '2 years', 'B', TRUE
...
SELECT t.id,
    '1FT' || UPPER(SUBSTR(MD5(RANDOM()::TEXT), 1, 14)),
...
SELECT t.id, v.id, v.assigned_driver_id, NOW() - (n * INTERVAL '1 day'), (15 + RANDOM() * 20)::NUMERIC(10,2), (50 + RANDOM() * 80)::NUMERIC(10,2),
    3.50::NUMERIC(10,4), v.current_mileage - (n * 100),
    CASE (n % 3) WHEN 0 THEN 'Shell Station' WHEN 1 THEN 'BP Station' ELSE 'Sunoco Station' END,
    v.fuel_type, 'FC-STG-' || LPAD(n::TEXT, 6, '0')
FROM temp_tenant t CROSS JOIN vehicles v CROSS JOIN generate_series(1, 4) n WHERE v.tenant_id = t.id AND v.fuel_type != 'Electric' LIMIT 100;

-- Insert work orders (50)
...
SELECT t.id, v.id,
    CASE (n % 10)
        ...
        WHEN 3 THEN 'Replace Battery'
        WHEN 4 THEN 'AC Repair'
        WHEN 5 THEN 'Coolant Flush'
        WHEN 6 THEN 'Transmission Service'
        WHEN 7 THEN 'DOT Inspection'
        WHEN 8 THEN 'Replace Brakes'
...
FROM temp_tenant t CROSS JOIN generate_series(1, 50) n CROSS JOIN LATERAL (SELECT id, current_mileage FROM vehicles WHERE tenant_id = t.id ORDER BY RANDOM() LIMIT 1) v;

-- Insert maintenance records (75)
...
SELECT t.id, v.id,
    ...
    (150 + RANDOM() * 350)::NUMERIC(10,2),
    'Fleet Service Center',
    'Completed scheduled maintenance'
FROM temp_tenant t CROSS JOIN vehicles v CROSS JOIN generate_series(1, 3) n WHERE v.tenant_id = t.id LIMIT 75;
