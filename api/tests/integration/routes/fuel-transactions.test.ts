...
describe('fuel-transactions Routes Integration Tests', () => {
  ...
  describe('GET /:id', () => {
    ...
    it('should return 404 for non-existent item', async () => {
      const response = await request(app)
        .get('/api/fuel-transactions/99999')
        .set('Authorization', `Bearer ${authToken}`)
        .expect(404)
  ...
  describe('DELETE /:id', () => {
    it('should delete item', async () => {
      await request(app)
        .delete('/api/fuel-transactions/1')
        .set('Authorization', `Bearer ${authToken}`)
        .expect(200)
    })
  })
