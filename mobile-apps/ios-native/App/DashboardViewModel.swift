...
// MARK: - Dashboard View Model (Legacy File - Redirected)
// The actual DashboardViewModel implementation is in ViewModels/DashboardViewModel.swift
// This file redirects to avoid breaking existing imports

// Re-export the actual DashboardViewModel from ViewModels folder
// typealias DashboardViewModel = ViewModels.DashboardViewModel

// Note: Since all files are in the same module, this file is kept minimal
// to avoid naming conflicts. The DashboardView will automatically find
// the DashboardViewModel class from ViewModels/DashboardViewModel.swift
//
    ...
    private func generateRecentActivity() {
        ...
        for trip in todayTrips.prefix(3) {
            activities.append(ActivityItem(
                ...
                description: "Vehicle \(trip.vehicleNumber) - Driver \(trip.driverId)",
        ...
        let maintenanceDue = vehicles.filter { vehicle in
            !vehicle.nextService.isEmpty
        }.prefix(2)
        for vehicle in maintenanceDue {
            activities.append(ActivityItem(
                ...
                description: "Vehicle \(vehicle.number) - \(vehicle.make) \(vehicle.model) - Next: \(vehicle.nextService)",
