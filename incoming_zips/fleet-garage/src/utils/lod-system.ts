...
export async function compressTexture(
  ...
  if (texture.image.width > settings.maxSize || texture.image.height > settings.maxSize) {
    const canvas = document.createElement('canvas');
    const ctx = canvas.getContext('2d')!;

    const scale = Math.min(
      settings.maxSize / texture.image.width,
      settings.maxSize / texture.image.height
    );
