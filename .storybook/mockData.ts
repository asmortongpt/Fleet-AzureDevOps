...
export function generateMockVehicles(count: number = 10): Vehicle[] {
  ...
  return Array.from({ length: count }, (_, i) => {
    const type = vehicleTypes[i % vehicleTypes.length] ?? "sedan"
    const status = statuses[i % statuses.length] ?? "idle"
    const fuelType = type === "bus" ? "diesel" : (fuelTypes[i % fuelTypes.length] ?? "gasoline")
    const make = makes[i % makes.length] ?? "Ford"
    ...
    return {
      ...
      tags: [type, fuelType, departments[i % departments.length] ?? "Operations"],
    }
  })
}
...
export function generateMockFacilities(count: number = 5): GISFacility[] {
  ...
  return Array.from({ length: count }, (_, i) => {
    const type = types[i % types.length] ?? "garage"
    return {
      id: "FAC-" + String(i + 1).padStart(4, "0"),
      tenantId: "tenant-001",
      name: names[i % names.length] ?? "Facility",
      type,
      status: (i % 7 === 0 ? "maintenance" : i % 11 === 0 ? "closed" : "operational") as GISFacility["status"],
      region: ["North", "South", "East", "West", "Central"][i % 5] ?? "Central",
...
export function generateMockCameras(count: number = 8): TrafficCamera[] {
  ...
  return Array.from({ length: count }, (_, i) => {
    const intersection = intersections[i % intersections.length] ?? { name: "Unknown Location", cross: "Unknown" }
