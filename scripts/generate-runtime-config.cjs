...
console.log(`🔧 Generating runtime-config.js for environment: ${environment}`);
...
try {
  ...
  console.error(`❌ Failed to write to public directory: ${error.message}`);
}
...
try {
  ...
  console.warn(`⚠️  Dist directory not found (this is OK during development): ${error.message}`);
}
