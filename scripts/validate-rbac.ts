...
interface RouteAnalysis {
  ...
  hasOldAuthorize: boolean
  hasFieldMasking: boolean
  authorizeCount: number
  requirePermissionCount: number
...
interface EndpointInfo {
  method: string
  path: string
  authType: 'requirePermission' | 'authorize' | 'none'
  permission?: string
  hasFieldMasking: boolean
}
...
interface ValidationReport {
  totalRoutes: number
  fullyUpdated: RouteAnalysis[]
  partiallyUpdated: RouteAnalysis[]
  notUpdated: RouteAnalysis[]
  needsFieldMasking: string[]
  summary: {
    fullCount: number
    partialCount: number
    noneCount: number
...
function extractEndpoints(content: string): EndpointInfo[] {
  ...
  while ((match = routeRegex.exec(content)) !== null) {
    ...
    let authType: 'requirePermission' | 'authorize' | 'none' = 'none'
...
function printReport(report: ValidationReport) {
  ...
  const oldAuthorizeEndpoints: Array<{file: string, endpoint: EndpointInfo}> = []
  report.notUpdated.forEach(route => {
